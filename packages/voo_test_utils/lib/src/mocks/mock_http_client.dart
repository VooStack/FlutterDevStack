...
class MockHttpClient {
  ...
  MockClient createRetryMockClient({
    int failCount = 2,
    int failStatusCode = 500,
    String successBody = '{"success": true}',
  }) {
    int attempts = 0;
    return MockClient((request) async {
      capturedRequests.add(CapturedRequest(request));
  ...
  CapturedRequest? get lastRequest =>
      capturedRequests.isNotEmpty ? capturedRequests.last : null;

  /// Gets all request bodies as a list.
  List<String> get requestBodies =>
      capturedRequests.map((r) => r.body).toList();

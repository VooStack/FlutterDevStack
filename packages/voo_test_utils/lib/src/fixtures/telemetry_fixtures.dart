...
class TelemetryFixtures {
  ...
  static List<Span> createSpanBatch({int count = 5}) => List.generate(
    count,
    (i) => Span(
      name: 'test-span-$i',
  ...
  static List<LogRecord> createLogBatch({int count = 5}) => List.generate(
    count,
    (i) => LogRecord(
  ...
  static (Span parent, Span child) createParentChildSpans() {
    ...
    final child = Span(name: 'child-span', traceId: parent.traceId, parentSpanId: parent.spanId, startTime: DateTime.now());
    return (parent, child);
  }

  /// Creates a completed span for testing.
  static Span createCompletedSpan({String name = 'completed-span', Duration duration = const Duration(milliseconds: 100)}) {
  ...
  static Span createErrorSpan({String name = 'error-span', String errorMessage = 'Test error occurred'}) {
  ...
  static Map<String, dynamic> sampleOtlpResource() => {
    'attributes': [
      ...
      },
      {
        ...
        'value': {'stringValue': 'voo_telemetry'},
      },
      {
  ...
  static Map<String, dynamic> sampleOtlpScope() => {'name': 'test-instrumentation', 'version': '1.0.0'};
}

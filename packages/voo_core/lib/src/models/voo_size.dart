...
class VooSize {
  ...
  VooSize operator *(double operand) => VooSize(width * operand, height * operand);

  /// Returns a size scaled by the inverse of the given [operand].
  VooSize operator /(double operand) => VooSize(width / operand, height / operand);

  /// Returns a size scaled by integer division.
  VooSize operator ~/(double operand) => VooSize((width ~/ operand).toDouble(), (height ~/ operand).toDouble());
  ...
  static VooSize lerp(VooSize a, VooSize b, double t) {
    return VooSize(a.width + (b.width - a.width) * t, a.height + (b.height - a.height) * t);
  }

  /// Returns a size that is at least as big as the given [minimum] size.
  VooSize constrain(VooSize minimum) {
    return VooSize(width < minimum.width ? minimum.width : width, height < minimum.height ? minimum.height : height);
  }
  ...
  factory VooSize.fromJson(Map<String, dynamic> json) => VooSize((json['width'] as num).toDouble(), (json['height'] as num).toDouble());

  @override
  bool operator ==(Object other) => identical(this, other) || other is VooSize && width == other.width && height == other.height;
